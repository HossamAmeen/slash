...
@php $input = "ar_postion"; @endphp
<div class="form-group">
  <label class="col-lg-2 control-label">الوظيفة</label>
  <div class="col-lg-10">
    <input type="text" name="{{ $input }}" @if(isset($row)) value="{{$row->$input}}" @else
      value="{{Request::old($input)}}" @endif class="form-control" required>
    @error($input)
...
@php $input = "postion"; @endphp
<div class="form-group"> 
  <label class="col-lg-2 control-label">الوظيفة بالانجليزي</label>
  <div class="col-lg-10">
    <input type="text" name="{{ $input }}" @if(isset($row)) value="{{$row->$input}}" @else
      value="{{Request::old($input)}}" @endif class="form-control" required>
    @error($input)
