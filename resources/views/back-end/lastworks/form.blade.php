...
<div class="form-group">
    ...
    <div class="col-lg-10">
        <select name="{{ $input }}" class="form-control">
          <option value="web" @if(isset($row) && str_contains($row->type , 'web')) selected @endif
            >web</option>
          <option value="branding" @if(isset($row) && str_contains($row->type , 'branding')) selected @endif>branding</option>
          <option value="socialMedia" @if(isset($row) && str_contains($row->type , 'socialMedia')) selected @endif
                >socialMedia</option>
          <option value="ecomerce" @if(isset($row) && str_contains($row->type , 'ecomerce')) selected @endif
                  >ecomerce</option>
        </select>
    </div>
</div>
